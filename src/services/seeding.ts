  ...
  try {
    ...
    if (users.length === 0) {
      ...
      const demoUsers: User[] = [
        { id: 'tl1', username: 'tl1', display_name: 'Marketing Leader 1', role: 'jpc_marketing', created_at: now() } as User,
        { id: 'sales1', username: 'sales1', display_name: 'Sales Person 1', role: 'jpc_sales', created_at: now() } as User,
        { id: 'cs1', username: 'cs1', display_name: 'CS Person 1', role: 'jpc_cs', created_at: now() } as User,
      ];
    ...
    const candidates = await apiService.getCandidates({ page: 1 });
    ...
    console.warn('Seeding failed (expected if API is empty/unreachable):', error);
  }
}
