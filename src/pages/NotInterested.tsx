...
export const NotInterested: React.FC = () => {
  ...
  const candidates = useMemo(() => {
    return rawCandidates.filter(c => c.current_stage === 'not_interested');
  }, [rawCandidates]);
  ...
  const handleRestore = async (candidate: Candidate) => {
    ...
    await logActivity(candidate.id, 'Candidate restored', 'Restored from Not Interested to Lead Generation.', user?.id ? String(user.id) : null);
  };
