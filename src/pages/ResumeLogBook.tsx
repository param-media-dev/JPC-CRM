...
import { ResumeChangeRequest, Candidate, User, ResumeVersion } from '../types';
import { 
  FileEdit, 
  Search, 
  Plus, 
...
export const ResumeLogBook: React.FC = () => {
  ...
  const [editingCompletedByReq, setEditingCompletedByReq] = useState<ResumeChangeRequest | null>(null);
  ...
  const handleAction = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (newStatus === 'completed' && finalResumeUrl) {
        const request = requests.find(r => r.id === requestId);
        if (request) {
          const candRef = doc(db, 'jpc_candidates', request.candidate_id);
          const candSnap = await getDoc(candRef);
          let finalVersions: ResumeVersion[] = [];
          
          if (candSnap.exists()) {
            const candData = candSnap.data() as Candidate;
            const existingVersions: ResumeVersion[] = candData.resume_versions ? [...candData.resume_versions] : [];
            if (existingVersions.length === 0 && (candData.resume_url || candData.resume_base64)) {
              existingVersions.push({
                id: `v1_${Date.now() - 1000}`,
                url: candData.resume_url || candData.resume_base64 || '',
                filename: candData.resume_filename || 'original_resume.pdf',
                uploaded_at: candData.updated_at || candData.created_at || new Date().toISOString(),
                version_number: 1,
                is_current: false,
                notes: 'Original resume'
              });
            }
            const updatedPrev = existingVersions.map(v => ({ ...v, is_current: false }));
            const newVerNum = updatedPrev.length + 1;
            const newVerObj: ResumeVersion = {
              id: `v${newVerNum}_${Date.now()}`,
              url: finalResumeUrl,
              filename: resumeFilename || `resume_v${newVerNum}.pdf`,
              uploaded_at: new Date().toISOString(),
              uploaded_by: activeCompletedById,
              uploaded_by_name: activeCompletedByName,
              notes: actionNotes || 'Resume patched and completed via Resume Log Book',
              version_number: newVerNum,
              is_current: true
            };
            finalVersions = [...updatedPrev, newVerObj];
          }
          ...
          await updateDoc(candRef, {
            ...
            resume_versions: finalVersions.length > 0 ? finalVersions : null,
            updated_at: new Date().toISOString()
          });
        }
      }
  ...
  const openEditCompletedBy = (req: ResumeChangeRequest) => {
  ...
  const handleSaveCompletedBy = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'jpc_resume_requests', editingCompletedByReq.id), {
  ...
  const handleUpdateStatus = async (
    requestId: string, 
    newStatus: ResumeChangeRequest['status'], 
    notes?: string, 
    resumeUrl?: string, 
    resumeBase64?: string, 
    ...
    try {
      ...
      if (newStatus === 'completed') {
        updateData.completed_by = completedBy || user?.id || null;
      ...
      if (request) {
        ...
        } else if (newStatus === 'pending_resume_team') {
          const resumeUser = team.find(u => u.role === 'jpc_resume');
          if (resumeUser) recipientId = resumeUser.id;
        }
  ...
  const handleExportReport = (statusToExport: string) => {
    const filtered = requests.filter(req => {
      ...
      if (statusToExport === 'pending_all') {
        return req.status === 'pending_tl' || req.status === 'pending_cs' || req.status === 'pending_resume_team';
      }
    ...
    const statusMap: Record<string, string> = {
      ...
      pending_resume_team: 'Pending Resume Team Action',
      completed: 'Completed',
      rejected: 'Rejected'
    };
    ...
    const exportData = filtered.map((req, idx) => {
      ...
      return {
        ...
        'Current / Original Resume URL': candidate?.resume_url || candidate?.resume_base64 || '',
    ...
    const fileName = `Resume_Log_Report_${statusLabel}_${new Date().toISOString().slice(0, 10)}.xlsx`;
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3 p-3.5 bg-accent-green/5 border border-accent-green/20 rounded-2xl">
                        <div className="flex flex-wrap items-center gap-3">
                          <div className="flex items-center gap-2 text-xs text-accent-green font-bold">
                            ...
                            <span className="text-text-secondary">Completed By:</span>
                            <span className="text-text-primary font-black text-sm">{getCompletedByName(req)}</span>
                            <span className="text-[10px] text-accent-green font-bold bg-accent-green/15 px-2 py-0.5 rounded-full border border-accent-green/20">
                              Resume Team
                            </span>
                          </div>
                        ...
                        {(user?.role === 'jpc_resume' || user?.role === 'administrator' || user?.role === 'jpc_sysadmin' || user?.role === 'jpc_manager') && (
                          <button
                            ...
                            className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-bold text-accent-green hover:bg-accent-green/15 border border-accent-green/30 rounded-xl transition-all cursor-pointer"
                            title="Update who completed this request"
                          >
                        ...
                        {req.status === 'completed' ? (
                          <div className="flex flex-col items-center gap-1.5 text-accent-green">
                            ...
                            <div className="text-center">
                              ...
                              <span className="text-[10px] text-accent-green font-bold block">
                                (Resume Team)
                              </span>
                            </div>
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-text-primary">Export Resume Log</h2>
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                >
                  ...
                  <option value="pending_resume_team">Pending Resume Team ({requests.filter(r => r.status === 'pending_resume_team').length})</option>
              ...
              <div className="p-4 bg-bg-tertiary rounded-2xl border border-border-primary/50 text-xs text-text-secondary space-y-1.5">
                ...
                <ul className="list-disc list-inside space-y-1 text-[11px] text-text-muted">
                  ...
                  <li>All comments/remarks (TL, CS, Resume team)</li>
                  <li><strong>Completed By & Completed Date/Time</strong></li>
                  <li>Full change instructions & document links</li>
                </ul>
              </div>
